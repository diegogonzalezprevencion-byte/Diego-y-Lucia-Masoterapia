...
export async function POST(request: Request) {
  try {
    ...
    const comentarios = typeof body.comentarios === "string" ? body.comentarios.trim() : "";
    const { client, error: configError } = getSupabaseAdmin();
    ...
    if (!client) {
      ...
      const emailNotification = await sendAdminReservationNotification(reserva);
      ...
      return NextResponse.json({
        ...
        reserva,
        automation: {
          whatsappMessage: reservationWhatsAppMessage({ ...body, comentarios }),
          emailNotification,
          remindersPrepared: true
        }
      }, { status: 201 });
    }
    ...
    // Si la base de datos antigua aún no tiene la columna comentarios, la reserva no se bloquea.
    // Para guardar y ver comentarios en Supabase, ejecutar: alter table public.reservas add column if not exists comentarios text;
    if (error && /comentarios/i.test(error.message)) {
      const { comentarios: _comentarios, ...payloadWithoutComments } = payload;
      const retry = await client
        .from("reservas")
        .insert(payloadWithoutComments)
        .select()
        .single();
      data = retry.data;
      error = retry.error;
    ...
    const emailNotification = await sendAdminReservationNotification(data || payload);
