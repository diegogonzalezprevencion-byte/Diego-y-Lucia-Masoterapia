...
export async function GET() {
  ...
  if (!client) {
    ...
    return NextResponse.json({
      ...
      reservas: [...store.reservas].sort((a, b) => (b.created_at || "").localeCompare(a.created_at || ""))
    });
  }
  ...
  return NextResponse.json({ ok: true, mode: "supabase", reservas: data }, { headers: { "Cache-Control": "no-store" } });
}
  ...
  try {
    ...
    if (!client) {
      ...
      return NextResponse.json({
        ...
        reserva,
        automation: {
          whatsappMessage: reservationWhatsAppMessage({ ...body, sucursal, masoterapeuta, comentarios }),
          emailNotification,
