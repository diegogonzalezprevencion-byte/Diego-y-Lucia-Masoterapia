  ...
  async function updateStatus(id: string, estado: string) {
    ...
    try {
      ...
      if (estado === "confirmada") {
        if (emailStatus?.sent) {
          setSuccess("Reserva confirmada y correo de confirmación enviado al cliente.");
        } else if (emailStatus?.skipped) {
          setSuccess(`Reserva confirmada. Correo no enviado: ${emailStatus.reason}`);
        } else if (emailStatus?.error) {
          setSuccess(`Reserva confirmada, pero no se pudo enviar el correo: ${emailStatus.error}`);
        } else {
          ...
          <tbody>
            {reservas.map((r) => (
              <tr key={r.id}>
                ...
                <td>{r.area}</td>
                <td>{r.servicio}</td>
                <td>{r.sucursal || "Santiago Centro"}<br />{r.masoterapeuta || "Diego González"}</td>
                <td>{r.nombre}</td>
                <td>{r.email}<br />{r.telefono}</td>
                <td>{r.comentarios?.trim() ? r.comentarios : "Sin comentarios"}</td>
                <td><span className={`status-pill status-${r.estado}`}>{r.estado}</span></td>
                <td>
                  <div className="admin-actions">
                    <button
                      className="btn"
                      onClick={() => updateStatus(r.id, "confirmada")}
                      disabled={!!updating || r.estado === "confirmada"}
                    >
                      {updating === `${r.id}-confirmada` ? "Confirmando..." : "Confirmar"}
                    </button>
                    <button
                      className="btn gray"
                      onClick={() => updateStatus(r.id, "pendiente")}
                      disabled={!!updating || r.estado === "pendiente"}
                    >
                      {updating === `${r.id}-pendiente` ? "Actualizando..." : "Pendiente"}
                    </button>
                    <button
                      className="btn red"
                      onClick={() => updateStatus(r.id, "cancelada")}
                      disabled={!!updating || r.estado === "cancelada"}
                    >
                      {updating === `${r.id}-cancelada` ? "Cancelando..." : "Cancelar"}
                    </button>
                  </div>
                </td>
              </tr>
            ))}
            {!loading && reservas.length === 0 && <tr><td colSpan={10}>No hay reservas registradas.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
