...
export default function AdminAvailability() {
  ...
  async function load(selectedArea = area, selectedFecha = fecha, selectedBranch = branch, selectedTherapist = therapist) {
    setError("");
    try {
      const params = new URLSearchParams({
        ...
        masoterapeuta: selectedTherapist
      });
      const response = await fetch(`/api/admin/disponibilidad?${params.toString()}`, { cache: "no-store" });
  ...
  async function updateSlot(slot: string, disponible: boolean) {
    ...
    try {
      ...
      setMessage(disponible ? `Horario ${slot} activado para ${branch} con ${therapist}.` : `Horario ${slot} desactivado para ${branch} con ${therapist}.`);
      await load();
    } catch (err: any) {
  ...
  async function updateFullDay(disponible: boolean) {
    ...
    try {
      ...
      setMessage(disponible ? `Jornada completa activada para ${branch} con ${therapist}.` : `Jornada completa desactivada para ${branch} con ${therapist}.`);
      await load();
    } catch (err: any) {
  ...
  useEffect(() => {
    load(area, fecha, branch, therapist);
  }, [area, fecha, branch, therapist]);
  ...
  return (
    <div className="grid-2">
      <div className="card">
        ...
        <p>
          Selecciona fecha, sucursal y masoterapeuta. Activa o desactiva cada bloque horario. Los horarios desactivados no aparecerán en la reserva pública.
        </p>
        <div className="form">
      ...
      <div className="card">
        ...
        <p>
          Control directo para el {fecha || "día seleccionado"}, sucursal {branch}, masoterapeuta {therapist}. Cada bloque dura 1 hora.
        </p>
