...
export default function BookingForm({ type, title, services }: BookingFormProps) {
  ...
  const selectedOffer = useMemo(() => {
    const selectedMonth = date ? Number(date.slice(5, 7)) : 0;
    return monthlyOffers.find((offer) => offer.mes === selectedMonth && offer.activo) || null;
  }, [date, monthlyOffers]);
    ...
    <div className="booking-stack-layout">
      <div className="card booking-calendar-card">
        <div className="booking-section-heading">
          <h2>{title}</h2>
          <p>Selecciona primero el día y horario disponible. Después completa tus datos para enviar la reserva.</p>
        </div>

        <div className="booking-calendar-block">
          <div className="booking-subheading">
            <strong>Selecciona un día</strong>
            <p className="small-helper">Calendario móvil de 30 días. Marca una fecha y luego elige un horario disponible.</p>
          </div>
        ...
        <div className="booking-slots-panel">
          <div className="booking-subheading">
            ...
            {date && <p className="small-helper">Fecha seleccionada: {date}</p>}
          </div>
      ...
      <div className="card booking-contact-card">
        <div className="booking-section-heading">
          ...
          <p>Indica el servicio que deseas, la sucursal, el masoterapeuta y tus datos de contacto para completar la solicitud.</p>
        </div>
