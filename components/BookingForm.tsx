...
export default function BookingForm({ type, title, services }: BookingFormProps) {
  ...
  useEffect(() => {
    async function loadSlots() {
      ...
      try {
        const response = await fetch(`/api/disponibilidad?area=${encodeURIComponent(type)}&fecha=${encodeURIComponent(date)}`, { cache: "no-store" });
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setName(""); setEmail(""); setPhone(""); setComments(""); setTime("");
