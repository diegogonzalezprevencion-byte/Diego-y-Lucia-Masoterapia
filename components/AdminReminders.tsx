...
export default function AdminReminders() {
  ...
  async function load() {
    ...
    setError("");
    try {
      const r = await fetch(`/api/admin/recordatorios?type=${filter}`, { cache: "no-store" });
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id + item.reminderType}>
                <td>
                  {item.fecha}<br />{item.hora}<br />{item.sucursal || item.area}<br />{item.masoterapeuta || ""}
                </td>
                <td>{item.nombre}<br />{item.email}<br />{item.telefono}</td>
                <td><span className="reminder-pill">{item.reminderType}</span></td>
                <td>{item.reminderMessage}</td>
                <td>
                  <div className="admin-actions">
                    <a
                      className="btn whatsapp"
                      href={buildWhatsAppUrlForNumber(item.telefono, item.reminderMessage)}
                      target="_blank"
                    ...
                    {(item.reminderType === "24h" || item.reminderType === "same_day") && (
                      <button className="btn gray" onClick={() => markSent(item.id, item.reminderType)}>
                        Marcar enviado
                      </button>
                    )}
                  </div>
